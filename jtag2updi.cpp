...
namespace
{
    ...
    inline void loop()
    {
#if (defined(USE_HV_PROGRAMMING) && (defined(__AVR_ATmega328P__) || defined(__AVR_ATmega_Mega__) || defined(__AVR_ATtiny_Zero_One__) || defined(__AVR_ATmega_Zero__) || defined(__AVR_DA__)))
        SYS::setPOWER();
        if (SYS::checkHVMODE() > 100)
        { // if HV or PCHV mode, then apply HV pulse and UPDI enable sequence
            SYS::pulseHV();
            SYS::updiEnable();
        }
        SYS::checkOVERLOAD();
...
// Receive command
            ...
            { // timed out waiting for host communication
...
// Time to give up on host, restore default baud rate, and wait for future contact
                    ...
                    { // send a fail and give them a chance to start talking again
    ...
    inline void process_command()
    {
        ...
        case JTAG2::CMND_SIGN_OFF:
            ...
            set_status(JTAG2::RSP_OK);
#if (defined(USE_HV_PROGRAMMING) && (defined(__AVR_ATmega328P__) || defined(__AVR_ATmega_Mega__) || defined(__AVR_ATtiny_Zero_One__) || defined(__AVR_ATmega_Zero__) || defined(__AVR_DA__)))
            if (SYS::checkHVMODE() > 200)
                SYS::cyclePOWER(); // if PCHV mode, power-cycle target
#endif
            break;
