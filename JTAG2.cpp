...
bool JTAG2::receive()
{
    ...
    if ((uint16_t)(JICE_io::get() | (JICE_io::get() << 8)) != crc)
        return false;
    return true;
}
...
void JTAG2::enter_progmode()
{
    ...
    case 0x82:
        ...
        if (!UPDI::CPU_reset())
        { // if we timeout while trying to reset, we are not communicating with chip, probably wiring error.
...
namespace
{
    ...
    void NVM_v2_write(uint32_t address, uint16_t length, uint8_t write_cmd)
    {
        ...
        if (length == 1)
        { // if just one byte write it with no looping
            // write to memory
        ...
        else
        {
            ...
            { // word write
                ...
                if (length & 0x01)
                { // in case they send us odd-length write command...
                    UPDI::stinc_b(JTAG2::packet.body[current_byte_index++]);
                }
